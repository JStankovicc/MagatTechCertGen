...
public interface SlozivoMeriloRepository extends JpaRepository<SlozivoMerilo, Integer> {
    ...
    @Query(value = "SELECT j.broj_zapisnika FROM _slozivo_merilo j WHERE j.broj_zapisnika LIKE %:broj% ORDER BY CAST(SUBSTRING(j.broj_zapisnika, INSTR(j.broj_zapisnika, 'K') + 1) AS UNSIGNED) DESC LIMIT 1", nativeQuery = true)
