...
public interface MernaTrakaSaViskom25mRepository extends JpaRepository<MernaTrakaSaViskom25m, Integer> {
    ...
    @Query(value = "SELECT j.broj_zapisnika FROM _merna_traka_sa_viskom_25m j WHERE j.broj_zapisnika LIKE %:broj% ORDER BY CAST(SUBSTRING(j.broj_zapisnika, INSTR(j.broj_zapisnika, 'K') + 1) AS UNSIGNED) DESC LIMIT 1", nativeQuery = true)
    String findBiggestBrojZapisnika(@Param("broj") String broj);

    List<MernaTrakaSaViskom25m> findAllByBrojZapisnikaLike(String formattedBrojSeta);
}
